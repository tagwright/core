...
with no per-network inspect. `ListNetworks` is also exercised against a live
Docker socket by `TestProvisionerDockerRoundTrip` below, which asserts a created
network's `Internal` flag through it.
